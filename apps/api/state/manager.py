...
class StateManager:
    ...
    def archive_task(self, project: str, task_name: str) -> dict:
        ...
        if task.get("archived_at"):
            return {"error": f"태스크 '{task_name}'은(는) 이미 아카이브되었습니다."}
        ...
        return {
            "success": True,
            "project": project,
            "archived_task": task_name,
        }

    def restore_task(self, project: str, task_name: str) -> dict:
        """아카이브된 태스크를 복구합니다."""
        data = self._load()
        if project not in data["projects"]:
            return {"error": f"프로젝트 '{project}'을(를) 찾을 수 없습니다."}
        ...
        if not task.get("archived_at"):
            return {"error": f"태스크 '{task_name}'은(는) 아카이브되지 않았습니다."}
...
'''
    ...
    def _get_jira_issue_tree(self, issue_key: str, auth, headers, visited: set, max_depth: int, depth: int = 0, lock=None, on_progress=None) -> dict:
        ...
        try:
            ...
            # Parent (상위 이슈) - depth 0일 때만 동기적으로 조회
            parent_result = None
            parent_field = fields.get("parent")
            if parent_field and depth == 0:
                parent_key = parent_field.get("key")
                if parent_key:
                    # visited 체크만 하고, 실제 추가는 _get_jira_issue_tree 내부에서 처리
                    with lock:
                        already_visited = parent_key in visited
                    if not already_visited:
                        # 동기적으로 parent 조회 (재귀 함수가 visited 처리)
                        parent_result = self._get_jira_issue_tree(
                            parent_key, auth, headers, visited, max_depth, depth, lock, on_progress
                        )

            # Subtasks - visited 체크만 하고, 실제 추가는 재귀 함수 내부에서
            subtasks = fields.get("subtasks", [])
            for st in subtasks:
                st_key = st.get("key")
                if st_key:
            ...
            if tasks:
                ...
                def fetch_child(task_info):
                    ...
                    next_depth = depth + 1
                    child_result = self._get_jira_issue_tree(key, auth, headers, visited, max_depth, next_depth, lock, on_progress)
    ...
    def _tree_to_graph_data(self, tree: dict, notion_titles: dict | None = None) -> dict:
        ...
        def process_node(node: dict, parent_key: str | None = None, link_type: str | None = None, depth: int = 0):
            ...
            # Parent (상위 이슈) 처리 - depth를 -1로 해서 더 크게 표시
            parent = node.get("parent")
            if parent and not parent.get("_skipped") and not parent.get("_error"):
                parent_key_val = parent.get("key")
                if parent_key_val and parent_key_val not in visited:
                    # Parent 먼저 처리 (depth -1로 더 상위에 표시)
                    process_node(parent, None, None, depth - 1)
                    # Parent → Current 링크 추가
                    link_id = f"{parent_key_val}->{key}"
                    existing_links = {f"{l['source']}->{l['target']}" for l in links}
                    if link_id not in existing_links:
                        links.append({
                            "source": parent_key_val,
                            "target": key,
                            "type": "parent",
                            "dashed": False,
                        })
