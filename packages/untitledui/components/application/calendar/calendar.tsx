...
const MonthView = ({
  ...
  const visibleEvents = zonedEvents.filter((event) => {
    ...
    return (
      eventStartDay.compare(weekEnd) <= 0 && eventEndDay.compare(weekStart) >= 0
    );
  });
                ...
                {eventsToShow.map(({ event, span, isPlaceholder }) => {
                  ...
                  return (
                    <div
                      key={event.id}
                      style={spanStyle}
                      className={cx(!isCurrentMonthFlag && "opacity-60")}
                    >
                      <CalendarMonthViewEvent
                        label={event.title}
                        collapseOnMobile={true}
    ...
    <div className={cx("relative flex-1", className)}>
      ...
      {dayEvents.map((event, index) => {
        ...
        const totalOverlapping = dayEvents.filter(
          (e) =>
            event.start.compare(e.end) < 0 && event.end.compare(e.start) > 0,
        ).length;
...
const WeekView = ({
  ...
  useLayoutEffect(() => {
    ...
    if (
      ...
      const startMinutes =
        earliestEventTimeInWeek.hour * 60 + earliestEventTimeInWeek.minute;
            ...
            <AriaCalendarGrid weekdayStyle="short" className="w-max">
              ...
              <AriaCalendarGridBody className="[&_tr]:last-of-type]:border-none [&_td]:p-0 [&_tr]:border-b-4 [&_tr]:border-transparent">
                {(date) => (
                  <CalendarCell
                    date={date}
                    isHighlighted={highlightedDates.has(date.toString())}
                  />
                )}
