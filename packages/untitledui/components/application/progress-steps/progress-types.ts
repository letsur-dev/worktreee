...
export type ItemsType<T extends ComponentType> = T extends "featured-icon"
  ? Step & { icon: IconType }
  : Step & { icon?: IconType };
...
export interface ProgressIconsCenteredProps<T extends ComponentType>
  extends Omit<CommonProps, "items"> {
  type?: T;
  connector?: boolean;
  items: ItemsType<T>[];
}
